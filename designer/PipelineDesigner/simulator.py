...
class Delay(object):
    def __init__(self, cause, cost, source=None):
        self.cause = cause
        self.cost = cost
        self.source = source
    ...
    def __str__(self):
        if self.cost == 0:
            return ""
        source = "due to " + str(self.source)
        if not self.source:
            source = ""
        return f"{self.cause} {self.cost} {source}"
...
class SimulatedTask(object):
    def __init__(self, name, image, ticks, needs, tags, stage, pipeline):
        ...
        self.delays = []
        self.runner = None
        self.runner_occupancy = 0

    def clone(self):
        copy = SimulatedTask(self.name, self.image, self.cost, self.needs, self.tags, self.stage, self.pipeline)
        copy.runner = self.runner
        copy.runner_occupancy = self.runner_occupancy
        copy.delays = list(self.delays)
        return copy
    ...
    def add_delay(self, reason, cost, source=None):
        self.delays.append(Delay(reason, cost, source))
...
class SimulatedRunner(object):
    ...
    def occupancy(self):
        return len(self.active_tasks()) / self.concurrent

    def could_execute(self, task):
        if task.name in self._could_run_cache:
            return self._could_run_cache[task.name]

        if self.images == task.image:
            for tag in task.tags:
                if tag not in self.tags:
                    self._could_run_cache[task.name] = False
                    return False
            self._could_run_cache[task.name] = True
            return True
    ...
    def can_execute(self, task):
        if self.concurrent > len(self._active):
            return self.could_execute(task)
        return False
    ...
    def execute(self, task, now):
        ...
        task.runner = self
        task.runner_occupancy = self.occupancy() - before
...
class SimulatedResources(object):
    ...
    def load_tasks(self, loader):
        """
        Populate the task list from the emulator loader
        :param loader:
        :return:
        """
        self.loader = loader
    ...
    def _stage_needs(self, task):
        """
        Get the tasks in earlier stages (for tasks that do not have needs)
        :param task:
        :return:
        """
        if not task.needs:
            ...
            if need_stages:
                ...
                result = list(earlier_stage_tasks)
                return result
        return []
            ...
            for task in self.tasks:
                ...
                for runner in self.runners:
                    if runner.can_execute(task):
                        ...
                        break
                if not started:
                    task.add_delay("runner", 1, source=task.runner)
